...
def create_stu_group(dis_group: dataclass, dgr_id: int) -> StuGroups | None:
    ...
    """
    if not (new_group := StuGroups.get_or_none(dgr_id=dgr_id)):
        ...
        try:
            ...
            return new_group
        except StuGroups as e:
            raise StuGroups(e.args)
...
def create_tp_delivery(tpdl_id: int, tp_delivery: dataclass, teach_program: dict) -> TpDeliveries | None:
    ...
    if not (delivery := TpDeliveries.get_or_none(tpdl_id=tpdl_id)):
        ...
        try:
            ...
            return new_delivery
        except TpDeliveryExc as e:
            raise TpDeliveryExc(e.args)
...
def create_tpr_chapters(tpdl_id: int, tc_chapters: dict) -> list:
    ...
    for tc_id in related_tc_ids:
        if tc_id not in existing_tpr_chapters_map:
            ...
            except TprChapterExc as e:
                raise TprChapterExc(e.args)
        else:
...
def create_tc_time(tc_id: int, ty_id: int, tc_time: dict, tch_id: int) -> list:
    ...
    for totc_id in related_totc_ids:
        if totc_id not in existing_tc_times_map:
            ...
            except TcTimeExc as e:
                raise TcTimeExc(e.args)
        else:
...
def create_teach_program(tp_id: int, teach_program: dict) -> TeachPrograms:
    ...
    """
    if not (program := TeachPrograms.get_or_none(tpr_id=tp_id)):
        try:
        ...
        except TeachProgramExc as e:
            raise TeachProgramExc(e.args)
...
def create_discipline(dis_id: int, discipline: dataclass) -> Disciplines:
    ...
    if not (disc := Disciplines.get_or_none(dis_id=dis_id)):
        try:
        ...
        except DisciplineExc as e:
            raise DisciplineExc(e.args)
...
def create_version() -> Versions:
    ...
    except VersionExc as e:
        raise VersionExc(e.args)
...
def create_group_work(sgr_id: int, type_of_work_id: int) -> GroupWorks:
    ...
    except GroupWorkExc as e:
        raise GroupWorkExc(e.args)
...
def create_group_faculty(
    ...
    for info, value in dif_group_fuculty.items():
        ...
        except GroupFacultyExc as e:
            raise GroupFacultyExc(e.args)
...
def dpv_branch(**kwargs) -> TpDeliveries | None:
    ...
    low_dgr_id: list = child_dgr_id(kwargs["dis_groups"], kwargs["dgr_id"])
    ...
    if len(tpdl_tp_dict) == 0:
        raise TpDeliveryExc(f"Нет студентов на нижнем уровне  {low_dgr_id}")
    ...
    if not checker(
            ...
            sorted_tc_time=kwargs["sorted_tc_time"],
            dss_id=kwargs["dss_id"]
    ):
        raise TpDeliveryExc(f"Разная учебная нагрузка у студентов групп(-ы) {low_dgr_id}")
...
@timer
def main():
    #logger.info("---START OF LOADING---")

    start_time = datetime.now()
    bar = tqdm(desc=f"[*] Выгрузка таблиц", total=19)
    ...
    bar.update(1)
    tc_time_for_check = get_tc_time_for_check(tc_time=time_of_tpd_chapters)
    bar.update(1)
    ...
    # Создаем VERSION
    version = Versions.get_or_none(ver_id=89)
    ...
    for dgr_id, dis_group in dis_groups.items():
        ...
        if dis_study.foe_foe_id not in config.ACCEPTABLE_FOE:  # если это не очная форма обучения, пропускаем
            bar.update(1)
            continue
        ...
        with database.atomic() as transaction:
            try:
                ...
                if not (tp_delivery := choice_of_branch(
                        ...
                        sorted_tc_time=tc_time_for_check
                )
                ):
                ...
                for typ_id, tpr_chapter in zip(get_ty_periods(dis_group.dss_dss_id, dgr_periods), tpr_chapters):
                    ...
                    try:
                        dgr_period = DgrPeriods.create(
                            ...
                            div_div=div_for_dgr if div_for_dgr else config.BASE_DIV,  # если кафедрру нет, то весим на ПГНИУ
                            typ_typ=typ_id
                        )
                    except DgrPeriodExc as e:
                        raise DgrPeriods
                    # Создаем TC_TIME
                ...
                logger.info(f"SUCCESS: для STU_GROUP {stu_group.sgr_id} была успешно создана из DIS_GROUP {dgr_id}")
                transaction.commit()

            except StuGroupExc as e:
                logger.warning(f'ALERT: ошибка во время создания StuGroup {e} для DIS_GROUP с dgr_id {dgr_id}')
                transaction.rollback()

            except TpDeliveryExc as e:
                logger.warning(f'ALERT: ошибка во время создания TpDelivery {e} для DIS_GROUP с dgr_id {dgr_id}')
                transaction.rollback()

            except TprChapterExc as e:
                logger.warning(f'ALERT: ошибка во время создания TprChapter {e} для DIS_GROUP с dgr_id {dgr_id}')
                transaction.rollback()

            except DgrPeriodExc as e:
                logger.warning(f'ALERT: ошибка во время создания DgrPeriod {e} для DIS_GROUP с dgr_id {dgr_id}')
                transaction.rollback()

            except TeachProgramExc as e:
                logger.warning(f'ALERT: ошибка во время создания TeachProgram {e} для DIS_GROUP с dgr_id {dgr_id}')
                transaction.rollback()

            except TcTimeExc as e:
                logger.warning(f'ALERT: ошибка во время создания TcTime {e} для DIS_GROUP с dgr_id {dgr_id}')
                transaction.rollback()

            except GroupWorkExc as e:
                logger.warning(f'ALERT: ошибка во время создания GroupWork {e} для DIS_GROUP с dgr_id {dgr_id}')
                transaction.rollback()

            except GroupFacultyExc as e:
                logger.warning(f'ALERT: ошибка во время создания GroupFaculty {e} для DIS_GROUP с dgr_id {dgr_id}')
                transaction.rollback()

            except DisciplineExc as e:
                logger.warning(f'ALERT: ошибка во время создания Discipline {e} для DIS_GROUP с dgr_id {dgr_id}')
                transaction.rollback()
